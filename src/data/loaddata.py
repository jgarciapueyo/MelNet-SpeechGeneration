...
The problem is that audio datasets samples are of various lengths, so torch.utils.data.DataLoader()
needs a way to collate (concatenate) the different samples of the batch into a single tensor, and
...
class AudioCollatePodcast():
    ...
    lengths by padding with zeros so that all samples have the same shape.
    ...
    def __call__(self, batch_samples: List[Tensor]) -> Tuple[Tensor, List[str]]:
        ...
        Args:
            batch_samples (List[Tensor]): list of the samples of the batch with shape:
